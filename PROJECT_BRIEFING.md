...
**Autonomous clock ticking (v0.9.59)**: Each scene, every unfilled threat clock (not owner-controlled) rolls 20% chance (`AUTONOMOUS_CLOCK_TICK_CHANCE`) to tick forward 1 segment. Logged and appended to `clock_events`. **NPC agency clocks (v0.9.83)**: `check_npc_agency()` now matches clock owner by normalized NPC name + aliases (was: `npc_id` comparison, which was always `False` since the AI writes names not IDs — scheme clocks never ticked via agency). **Load-time owner repair (v0.9.83)**: `load_game()` builds a normalized `alias → canonical name` map and updates any clock owner that resolves to a different canonical name, healing stale owners from saves made before v0.9.83.
