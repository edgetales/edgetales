...
8. **Director guidance** — captured from `game.director_guidance` **only when `director_ran=True`** (stale guidance from previous calls suppressed): narrator_guidance, pacing, arc_notes, `act_transition` (bool), `npc_reflections[]` (npc_id, tone, tone_key, updated_arc). Both `act_transition` and `npc_reflections` are now stored in `game.director_guidance` by `_apply_director_guidance()` (v0.9.90 fix) — previously only narrator_guidance/pacing/arc_notes were stored, making these fields unreadable after `run_deferred_director()` returned.
...
- `final_state` — character, chapter, location, scene, health, spirit, supply, momentum, chaos, npcs, active_clocks, `triggered_transitions` (list of act IDs confirmed by Director or back-filled, from the live blueprint at session end), `triggered_director_phases` (list of phase keys that fired a Director call, for deduplication audit — from the live blueprint at session end)
...
- `director_guidance` — present **only when `director_ran=true`** (stale guidance from previous calls is suppressed): narrator_guidance, pacing, arc_notes, `act_transition` (bool — whether the Director evaluated the current act's transition trigger as fulfilled), `npc_reflections[]` (npc_id, tone, tone_key, updated_arc for each NPC the Director reflected on)
...
**Director `instinct` diversity for mid-game NPCs (v0.9.88, reframed v0.9.90)**: Mid-game NPCs created via `new_npcs` start with `instinct: ""` and receive their instinct from the Director on the same turn. Note: `updated_instinct` was removed in v0.9.90; instinct is now set once and locked. The initial-fill applies via the `instinct` field (only when `needs_profile="true"`). **v0.9.90 reframe**: instruction now defines instinct as a *psychological signature under real pressure* — what the NPC specifically does when their strategy fails, what is slightly irrational but inevitable. Concrete BAD/GOOD examples replace the former spectrum list. See opening-metadata callsite for same reframe.
...
5. **`_process_npc_details()` identity-reveal fallback (v0.9.92)**: if `full_name` completely differs from current NPC name, an identity reveal is attempted — but only after a **Memory Guard**. Guard logic: if the existing NPC has at least one memory (= established character the player already knows) AND the new name shares zero normalised words with the existing name and all its aliases, the rename is **rejected** — the Brain has almost certainly confused two distinct characters (e.g. background NPC "Theo" with memories mis-mapped to newly introduced "Klaus Kinski"). Instead a fresh NPC stub is created for the new name (skipped if `_process_new_npcs()` already created it in the same cycle). NPCs with 0 memories (fresh stubs) pass through unconditionally. If the guard passes, `_merge_npc_identity()` is called followed by `_absorb_duplicate_npc()`. **`npc_renames` has no Memory Guard** — the Narrator is considered a more reliable source (it has seen the full prose) and its renames are always trusted. **Stub description fallback (v0.9.92)**: when the guard creates a stub and the `npc_details` entry has no description, `world_addition` from the Brain output is used as a seed. `world_addition` is threaded via `_apply_narrator_metadata(world_addition=...)` → `_process_npc_details(world_addition=...)`. All 4 `_apply_narrator_metadata` callsites pass `brain.get("world_addition", "") or ""` (momentum-burn path uses `brain_data`). **Extractor prompt hardened**: `npc_details` instruction now explicitly states it is for already-known NPCs only — new characters must always enter via `new_npcs` with a full description.
...
**NPC instinct — psychological signature reframe (v0.9.88 origin, v0.9.90 reframe)**: Root cause (v0.9.88): savegame analysis showed all NPCs with identical low-arousal/high-dominance profiles — Haiku's unconstrained default. First fix: spectrum list + phrase ban ("calm and calculating"). Second analysis (v0.9.90 Elvira run): Haiku found synonyms for the same emotional territory ("bedächtige Handlungskompetenz", "kühle methodische Effizienz") — textually distinct, emotionally identical. Root cause of fix failure: spectrum list produces label-thinking, not character-thinking. **v0.9.90 reframe**: both `call_opening_metadata()` and the Director `needs_profile` instruction now define instinct as the NPC's *psychological signature under real pressure* — what they specifically do when their strategy fails, what is slightly irrational but inevitable. Spectrum list and no-repeat constraint removed. BAD/GOOD concrete examples take their place. A SELF-CHECK test ("could this apply to any competent professional in this genre?") gives Haiku an actionable criterion. Genre-prior warning added to opening extractor: situational composure in the narration is not personality wiring — infer the person, not the performance.
...
| `director_guidance` | dict | Latest Director output — keys: `narrator_guidance`, `npc_guidance`, `pacing`, `arc_notes`, `act_transition` (bool), `npc_reflections` (list of `{npc_id, tone, tone_key, updated_arc}` — diagnostic subset; prose and write-back fields applied to NPC state are excluded). Reset to `{}` on chapter start. |
