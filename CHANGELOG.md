...
### Added
- **Mid-game clock creation via Director:** The Director can now propose up to two new threat clocks per run via a `new_clocks` array in `DIRECTOR_OUTPUT_SCHEMA`. `_apply_director_guidance()` deduplicates proposals against `game.clocks` by normalised name, assigns sequential IDs, and clamps `segments` to [2–10] and `filled` to [0–segments]. The Director prompt includes creation conditions (concrete new threat not yet tracked, urgency-scaled segments, trigger description) and the `<clocks>` tag shows a live slot annotation so the model has explicit budget context.
- **`MAX_ACTIVE_CLOCKS = 6` hard cap:** `_apply_director_guidance()` counts unfired active clocks before accepting Director-proposed clocks; proposals beyond the cap are dropped. The clock annotation shows `(N/6 active — cap reached, do NOT add new clocks)` or remaining slots.
- **Elvira: `time_of_day` freeze detection:** The main turn loop tracks consecutive scenes without a `time_of_day` change. Once the counter reaches `TIME_STALE_THRESHOLD = 10`, a named soft warning is appended to `turn_record["violations"]`. Not an invariant violation — frozen time can be valid — but surfaces consistently for session analysis.

### Changed
- **Combat moves now deal damage on WEAK_HIT:** A WEAK_HIT on `clash` or `strike` previously granted only `momentum +1` with no health cost. Fixed in `apply_consequences()`: WEAK_HIT on `clash`/`strike` now deducts `health -1` (risky) or `health -2` (desperate); controlled position exempt. Mirrors existing MISS damage scaling for those moves.

### Fixed

#### NPC System
- **NPC identity oscillation — established-character guard for `_process_npc_renames`:** `_process_npc_renames` had no protection against Extractor mis-routing. When an established NPC appeared adjacent to a newly mentioned character, the Extractor sometimes assigned the new character's name to the established NPC's `npc_id`, causing the slot to oscillate between two distinct characters every turn as both names accumulated in the alias list. Fixed: if the targeted NPC is established (`memory` present OR `agenda` set) AND has a proper name (not a descriptor placeholder like "The Stranger" — detected via new `_is_descriptor_name()` helper) AND the new name shares zero words with the current name and all aliases, the rename is rejected and a new NPC stub is created instead. Extended the same guard in `_process_npc_details` from `bool(memory)` to `bool(memory) or bool(agenda)` to also protect Opening Extractor NPCs that carry an agenda but may not yet have memories on the first turn of a chapter.

- **NPC title/honorific handling — complete system fix:** Title prefixes (Dr., Prof., Captain, Herr, etc.) were silently stripped or swapped through three independent rename paths. Added `_is_title_only_difference(old_norm, new_norm)` — strips `_NAME_TITLES` words from both sides and returns True when the non-title cores are identical, reusing the authoritative title set already used in fuzzy matching (EN/DE/FR/ES, academic + military + nobility + RPG). Applied to all three paths: `_process_npc_renames`, `_process_npc_details`, and `_process_new_npcs` (new `elif` branch alongside `stt_variant`). In all three: when a title-only difference is detected the more complete name is kept as primary and the shorter form registered as alias — no identity merge. Title-variant branches also call `_reactivate_npc()` when the NPC is background/lore, and process parenthetical aliases from the incoming name. Also added `_find_npc()` step 3b (title-aware core match) between alias exact-match and substring fallback: returns the NPC only when exactly one unambiguous core match exists, closing the gap for short-named NPCs where the ≥5-char substring floor fails. Extension check in `_process_npc_details` simplified: `or new_norm in old_norm` removed — the extension path now only fires in the forward direction; title differences are handled by `_is_title_only_difference` instead.

- **Opening Extractor NPCs missing seed memory:** NPCs created by `call_opening_metadata()` via `_process_game_data()` received no seed memory when the extractor omitted `memory_updates` for them, unlike mid-game NPCs from `_process_new_npcs()` which always get an auto-generated seed. Hollow opening NPCs had `memory=[]` and `last_memory_scene=0` for the entire campaign: invisible to the Director's reflection cycle, unprotected by identity-reveal guards, and never triggering staleness retirement. Fixed: `_process_game_data()` now creates a seed memory for any NPC whose `memory` list is empty after the extractor's data is applied. Seed uses the NPC's description (or fallback appearance notice), maps disposition to emotional weight, scores importance with a floor of 3, and sets `last_memory_scene = game.scene_count`.

- **Staleness-based NPC retirement (`NPC_STALE_SCENES = 8`):** `_retire_distant_npcs()` previously only demoted NPCs when the active count exceeded `MAX_ACTIVE_NPCS (12)`. NPCs with `bond=0` that had not received a memory update for ≥8 scenes were never retired, consuming context budget and adding TF-IDF noise. New staleness path: any active NPC with `bond=0` and `last_memory_scene` ≥ 8 scenes past is demoted to `background`. Guards: only fires after scene 8; `last_memory_scene == 0` treated as not stale; `bond > 0` always protects. `last_memory_scene` added to all NPC creation sites, `_ensure_npc_memory_fields()`, `load_game()` migration, and `_apply_memory_updates()`. `_retire_distant_npcs()` is now called exclusively from `_apply_narrator_metadata()` after `_apply_memory_updates()` so `last_memory_scene` is fresh. Reactivation guard: a reactivated NPC is protected from immediate re-retirement in the same scene via `has_current_scene_memory` (any memory with `scene == game.scene_count` exempts the NPC regardless of `last_memory_scene`).

- **Absent-NPC mention memories (`absent=true`):** The Extractor was forbidden from recording memory updates for off-screen NPCs, leaving plot-central characters with frozen memory state during absence. `NARRATOR_METADATA_SCHEMA.memory_updates` gains an optional `absent` boolean. When `absent=true`, the Extractor may record one off-screen memory for narratively significant off-screen developments. Application path: absent-mention memories bypass the presence guard, receive `importance=3`, `type="mention"`, do not reactivate background NPCs, do not update `last_location`, and do not increment `importance_accumulator`. Rate cap: max 2 absent-mention entries per scene; duplicates for the same NPC blocked; absent entries for deceased NPCs skipped; lore NPCs fall through to normal processing (already presence-guard-exempt). Absent-mention memories are included in the Director's `<reflect>` context so off-screen NPC developments reach arc synthesis. `absent=true` entries for unknown NPC IDs produce a WARNING log rather than dropping silently.

- **Director-driven organic disposition shifts:** `DIRECTOR_OUTPUT_SCHEMA.npc_reflections` gains a `disposition_shift` field (`"improve"` / `"worsen"` / `null`). `_apply_director_guidance()` applies the shift one step along `hostile→distrustful→neutral→friendly→loyal`; at-pole shifts are logged and skipped. `disposition_shift` is stored in `game.director_guidance["npc_reflections"]` for Elvira diagnostics. Prompt emphasises rarity: this reflects narrative accumulation across many scenes, not a reward for a single roll.

- **`compel` STRONG_HIT incorrectly incremented NPC bond:** `apply_consequences()` included `compel` in the STRONG_HIT bond-increment set. A successful compel is transactional — it does not deepen a relationship. Bond on STRONG_HIT now only increases for `make_connection` and `test_bond`, matching the pre-existing disposition-shift logic which already excluded `compel`.

- **Director wrote reflections for lore/deceased NPCs:** Despite a "Only for NPCs in `<reflect>` tags" instruction, the Director occasionally output ghost entries for lore NPCs. Fixed with defence in depth: `_apply_director_guidance()` now explicitly rejects reflections whose NPC status is not `active` or `background`; the Director prompt instruction explicitly names lore and deceased as context-only.

- **Slow/literary NPC deaths not marked `deceased`:** When an NPC died via gradual literary prose, the Extractor could describe the death in its own `scene_context` yet leave `deceased_npcs` empty. Fixed: CROSS-FIELD CONSISTENCY rule added to the Extractor prompt — if `scene_context` mentions an NPC dying or dead (English and German patterns), that NPC must appear in `deceased_npcs`.

- **NPC activation false positive on German articles:** `activate_npcs_for_prompt()` matched name parts with `len(part) >= 3`, passing `der`, `die`, `das`, `von` as activation signals. Raised to `len(part) >= 4`, consistent with the exit-suppression check in the same function.

- **NPC created and killed in the same scene:** The Extractor could not mark a same-scene death in `deceased_npcs` because no `npc_id` existed at extraction time. Fixed: `new_npcs` entries gain an optional `deceased: bool` flag. `_apply_narrator_metadata()` resolves the name after `_process_new_npcs()` and calls `_process_deceased_npcs()` after `_apply_memory_updates()` — ensuring memories are stored before the NPC is marked deceased.

- **Off-screen narrator-confirmed deaths not marked `deceased`:** The Extractor prompt's "physically-witnessed" wording implied the player character must have directly observed the death. Changed to "narrator-confirmed": the player need not witness the death; it suffices that the narrator unambiguously depicts or confirms the outcome.

- **Opening metadata failure left `game.npcs` empty:** When `call_opening_metadata()` hit a transient API error, the fallback returned an empty dict and `game.npcs` silently remained `[]` for the entire session. Fixed: `start_new_game()` and `start_new_chapter()` now retry once when `game.npcs` remains empty (or `scene_context` equals the pre-set placeholder) after the first call; ERROR logged if the retry also fails.

- **NPC agency ticks skipped on dialog turns:** `check_npc_agency()` was only called in the action sub-path of `process_turn()`. Dialog turns that fell on a multiple-of-5 scene permanently skipped the agency interval. Fixed: `check_npc_agency()` now called in the dialog path immediately before `_tick_autonomous_clocks()`.

- **Duplicate `npc_reflections` entries per NPC:** The Director schema had no uniqueness constraint on `npc_reflections`, allowing multiple entries with the same `npc_id` in a single response. This caused multiple reflection memories per NPC per turn and overwritten `arc`/`agenda` fields. Fixed: `successfully_reflected_ids` dedup guard added to `_apply_director_guidance()` — a duplicate is only skipped once the first entry has been successfully stored, allowing a valid second entry through if the first was rejected as truncated.

#### Time System
- **`time_of_day` freeze — two-level self-correction:** Two independent mechanisms could simultaneously stay silent in static-location scenarios. Engine-level correction: added `time_unchanged_scenes` counter to `GameState` (transient, not persisted), incrementing every scene when the Extractor returns no `time_update` and resetting on any time change (Extractor, `advance_time()`, chapter start, opening extractor). At `TIME_STALE_THRESHOLD = 5` scenes: `_time_ctx()` appends an inline directive to the Narrator's `<time>` tag; `build_director_prompt()` emits `<current_time stale_scenes="N">` and adds a TIME RULE to the `narrator_guidance` task field. Both corrections fire before the Elvira violation threshold (10 scenes). Prompt-level correction: the Narrator's TEMPORAL CONSISTENCY block gained a TEMPORAL TEXTURE rule (anchor one atmospheric time detail every 2–3 scenes even in static locations); the Extractor's `time_update` instruction now enumerates qualifying atmospheric signals and clarifies that explicit clock language is not required.

#### Director / Narrative Quality
- **`arc_notes` and `npc_guidance` dropped when `narrator_guidance` was empty:** Both were nested inside `if dg.get("narrator_guidance"):`. Fixed: all three Director output fields now checked independently at the `if dg:` level.

- **Internal clock names leaked into narrator prose:** The Director referenced clocks by internal name in `narrator_guidance`, and these names appeared verbatim in prose. Fixed: Director prompt instructs describing threats narratively, never by internal clock name. Narrator prompt gains a matching global rule.

- **`ketsu_resolution` phase stagnation:** The final Kishotenketsu act had no specific pacing instruction, allowing ongoing reflection rather than driving toward consequences. Fixed: Narrator gains a `ketsu_resolution` phase rule (each narration must move toward something irreversible); Director task gains a FINAL ACT RULE (push toward concrete irreversible action; inject a new variable if the player has circled the same question for two or more scenes).

- **NPC improvised biography:** The Narrator could invent family members, formative events, or personal history not established in the NPC's profile or any prior scene. New NPC BACKSTORY rule: NPCs may only reference people, events, or relationships present in their description, agenda, arc, or a prior scene.

- **Atmospheric motif repetition:** Recurring mood anchors (e.g. "Stille", "Kälte") became formulaic defaults in tone-consistent genres. Extended PHRASE VARIETY with an ATMOSPHERIC MOTIFS sub-rule: if the same motif has anchored two or more consecutive narrations, switch to a different sensory dimension.

- **German examples in AI prompts replaced with English:** Six prompt locations contained German-language examples sent to the models — clock names, PHRASE VARIETY patterns, a location example, and NPC rename/death dialog examples. All replaced with English equivalents.

#### Session / Metadata Stability
- **`scene_context` stale or empty after metadata failure:** Two independent gaps. (1) When `call_narrator_metadata()` failed transiently, the Python-level fallback returned `""`, silently leaving the previous turn's context in place. Fixed: one inline retry when `scene_context` is empty after a successful parse. (2) The schema lacked `minLength: 1`, allowing the model to return `""` via Structured Outputs; added to `NARRATOR_METADATA_SCHEMA` and `OPENING_METADATA_SCHEMA`.

- **Narrator gender inconsistency when backstory and character_concept conflict:** Gendered language in backstory was overridden by `character_concept` gender in early scenes. BACKSTORY CANON rule updated: gendered language in the backstory takes absolute priority.

- **Anthropic Structured Outputs API breaking change — `maxItems`/`minItems` constraints rejected:** `maxItems` on array types is no longer accepted; `minItems` only accepts 0 or 1. All `maxItems` constraints and `minItems: 3`/`minItems: 2` on `acts` and `scene_range` caused 400 errors on every affected call, silently killing the NPC system and Director for entire sessions. Fixed: all `maxItems` removed; invalid `minItems` values reduced. `_safe_sr()` promoted to module-level helper and applied at all 6 `scene_range` access sites. `call_story_architect()` now validates `len(acts) >= 3/4` after a successful response and triggers the context-aware fallback on structurally broken blueprints.
---

## [0.9.95]

### Added
- **Active clocks injected into Narrator prompt (`_narrator_clocks_block()`):** Narrator now receives all unfired, non-empty clocks as `<active_threats>` with fill bar and urgency descriptor (low / moderate / high / critical — thresholds at 40% / 65% / 85%). Previously visible only to Brain and Director; Narrator had no clock awareness and could not reflect threat pressure in prose. Instruction: translate urgency into sensation and atmosphere, never reference game terms. Block omitted when no active clocks exist or all are unfilled.
- **NPC cross-relationship block (`_npc_relations_block()`):** New function mines `about_npc` memories across all NPCs present in a scene and builds a `<npc_dynamics>` block. Previously only the target NPC's views of others were surfaced (via `npc_views` in `_npc_block`); activated NPCs' memories about each other were invisible to the Narrator. Deduplicated by NPC pair (one entry per pair), capped at 6 entries, most-recent memory first. Narrator instruction: let relationship history surface through behavior and subtext, not direct narration.
- **`arc_notes` forwarded from Director to Narrator:** Director's `arc_notes` field (story arc progress observation synthesized from full clock + NPC + scene-log context) was stored in `game.director_guidance` but never passed to Narrator prompts — a silent data-flow gap. Now appended to `director_block` in both `build_action_prompt()` and `build_dialog_prompt()`. Narrator instruction: background awareness only, not direct reference.
- **Director interval reduced from 3 → 2 (`DIRECTOR_INTERVAL`):** With `arc_notes`, clocks, and NPC dynamics now flowing from Director to Narrator, staleness of Director output becomes more impactful. Interval 2 halves the worst-case stale-context lag (from 2 scenes to 1) at negligible Haiku cost. Event-based triggers (MISS, chaos, new NPCs, reflections, phase changes) are unaffected.
- **Anti-verbosity and precision rule in Narrator system prompt (`PRECISION`):** Every sentence must carry new information, advance tension, reveal character, or deepen atmosphere. Generic atmospheric filler explicitly discouraged. New conditional rules added for all three new blocks: `<active_threats>` (translate urgency to sensation, never mention game terms), `<arc_notes>` (background awareness, not direct reference), `<npc_dynamics>` (surface through behavior, not narration).
- **Narrator system prompt caching (`cache_control: ephemeral`):** `call_narrator()` now sends the system prompt as a content block list with `cache_control: {"type": "ephemeral"}`, enabling Anthropic's server-side 5-minute prompt cache. To make the system prompt fully static per session (required for cache hits), `_status_context_block()` (H/Sp/Su → narrative state descriptions) was moved out of `get_narrator_system()` and injected directly into all four user-prompt builders (`build_dialog_prompt`, `build_action_prompt`, `build_new_game_prompt`, `build_new_chapter_prompt`) before `_story_context_block`. The system prompt now contains only truly static content (rules, backstory, tone, content boundaries) and qualifies for caching every time H/Sp/Su change — which was previously causing cache misses on every action scene. Expected savings: ~20% of Narrator input tokens at typical 2–3 min/turn play pace (cache survives 2–3 scenes per session on average, break-even at 2nd hit).

### Changed
- **`elvira_config.json` — `player_name` and `backstory` fields added:** Two new optional fields under `game` allow pre-defining the player character's name and backstory directly in the Elvira config. `player_name` is passed as a name override into the character-creation prompt (Narrator uses it verbatim). `backstory` is passed as a dedicated `creation_data` key; `engine.py` now prefers this key over `custom_desc` for the `GameState.backstory` field (fallback to `custom_desc` preserved for compatibility). Both fields default to empty string — existing behaviour unchanged when omitted.

### Fixed
- **`npc_reflections` duplicate guard placement bug:** The initial fix for duplicate `npc_reflections` entries used a separate `seen_reflection_npc_ids` set that was populated *before* validity checks (NPC lookup, truncation test). A valid second entry for the same `npc_id` would be blocked even if the first was rejected as truncated. Fixed: `seen_reflection_npc_ids` removed entirely; `successfully_reflected_ids` is now checked at loop entry instead — a duplicate is only skipped once the first entry has been successfully stored. This allows a valid second entry to be processed if the first failed, at no cost to the normal (no-duplicate) path.
- **Schema hardening — `maxItems` / structural constraints across all Structured Output schemas:** Systematic review revealed that most array fields in all five schemas had no size constraints, allowing models to return pathologically large or structurally malformed arrays that either waste tokens or cause unexpected state mutations. Added constraints: `NARRATOR_METADATA_SCHEMA` — `memory_updates` (20), `new_npcs` (8), `npc_renames` (5), `npc_details` (8), `deceased_npcs` (5), `lore_npcs` (5), `exited_npc_ids` (8). `OPENING_METADATA_SCHEMA` — `npcs` (10), `clocks` (6), `deceased_npcs` (5). `DIRECTOR_OUTPUT_SCHEMA` — `npc_guidance` (10). `STORY_ARCHITECT_OUTPUT_SCHEMA` — `acts` (minItems 3, maxItems 4), `scene_range` (minItems 2, maxItems 2 — prevents IndexError in `get_current_act()`), `revelations` (5), `possible_endings` (minItems 1, maxItems 5). `CHAPTER_SUMMARY_OUTPUT_SCHEMA` — `unresolved_threads` (6), `npc_evolutions` (15).
- **Duplicate `npc_reflections` entries per NPC in one Director run:** The Director schema placed no uniqueness constraint on `npc_reflections`, allowing Haiku to return multiple entries with the same `npc_id` in a single response (observed: 3 entries per NPC, 9 total). The processing loop in `_apply_director_guidance()` iterated all entries without a dedup guard, causing: multiple reflection memories written to the same NPC in one turn (burning reflection-slot budget in `_consolidate_memory()`), and `arc`/`agenda` fields overwritten multiple times with only the last value surviving. Fix: duplicate guard via `successfully_reflected_ids` (see above). Defence-in-depth: `maxItems: 10` added to the `npc_reflections` array schema.
- **Atmospheric motif repetition (PHRASE VARIETY extension):** The existing PHRASE VARIETY rule only tracked one specific syntactic crutch. Session analysis revealed that single sensory/mood anchors (e.g. "Stille") can become an atmospheric default in tone-consistent genres (horror, dark), appearing in every narration as formulaically as a structural crutch. Extended the rule with an ATMOSPHERIC MOTIFS sub-rule: if the same motif has anchored two or more consecutive narrations, it is treated as a crutch — the Narrator is instructed to switch to a different sensory dimension. Genre-specific example ("eine andere Stille") included.
- **NPC improvised biography (NPC BACKSTORY rule):** The Narrator could invent family members, formative events, or personal history for NPCs not established in their profile or any prior scene (observed: Teo's mother appearing without prior seeding). New NPC BACKSTORY rule: NPCs may only reference people, events, or relationships present in their description, agenda, arc, or a prior scene. Sparse NPC profiles should manifest as sparse presence, expressed through behavior — not filled in by invention.
- **`ketsu_resolution` phase stagnation:** The final Kishotenketsu act had no specific pacing instruction, allowing the Narrator and Director to continue circling the ten_twist's existential revelation instead of moving toward irreversible consequences. Two additions: (1) Narrator system prompt: new `ketsu_resolution` phase rule — the ten_twist has landed; each narration must now move toward something irreversible (choice made, system changed, door closed). (2) Director task: FINAL ACT RULE for `narrator_guidance` — in `ketsu_resolution`/`resolution` phases, guidance must push toward concrete irreversible action, not further reflection; if the player has circled the same question for two or more scenes, inject a new variable.
- **`arc_notes` and `npc_guidance` dropped when `narrator_guidance` is empty string:** Both fields were nested inside `if dg.get("narrator_guidance"):` in both prompt builders. An empty `narrator_guidance` (valid — Director may have context notes or NPC instructions without a concrete scene direction) caused `arc_notes` and all per-NPC guidance to be silently discarded. Fixed: all three Director output fields (`narrator_guidance`, `npc_guidance`, `arc_notes`) are now checked independently at the `if dg:` level — none can suppress the others.
- **`_narrator_clocks_block()` showed zero-fill clocks:** Clocks with `filled=0` were included in `<active_threats>`, adding noise without pressure value. Added `filled > 0` filter. Only clocks with at least one filled segment are shown — consistent with the Director's own practice of referencing clocks only when ≥50% filled.
- **`_npc_relations_block()` docstring incorrect:** Stated "Skips the target NPC" but all present NPCs including the target were processed. Docstring corrected; the minor overlap with `npc_views` in `_npc_block` is intentional — different memory selection criteria (recency vs. TF-IDF), different perspective on the same relationship.
- **Elvira: `npc_guidance` missing from Director log:** `turn_record["director_guidance"]` logged `narrator_guidance`, `arc_notes`, `pacing`, `act_transition`, and `npc_reflections` but omitted `npc_guidance`. Added so all Director output fields are visible in session JSON for diagnostics.
- **Clock names leaked into narrator prose:** The Director was instructed to reference clocks in `narrator_guidance`/`arc_notes` when ≥50% filled, but had no instruction to avoid using internal clock names (e.g. "Dorfmob-Uhr"). These names passed through `<director_guidance>` and appeared verbatim in narrator prose. Two-layer fix: (1) Director system prompt: new explicit rule — describe threats narratively ("the mob is getting closer"), never by internal clock name. (2) Narrator system prompt: new global rule immediately after "NEVER mention dice/stats" — never use internal clock names in prose regardless of where they came from.
- **German examples replaced with English in all AI prompts:** Six locations contained German-language examples that were being sent to the models: clock name examples in both Narrator and Director prompts (`"Dorfmob-Uhr"` → `"Mob Clock"`), PHRASE VARIETY pattern examples (`"mit dem Tonfall eines Mannes"` → `"with the calm of a man"`), location example in Metadata Extractor (`"Die Kathedrale"` → `"The Cathedral"`), npc_renames descriptor examples (`"Die Frau im Wollhut"` → `"The Woman in the Wool Hat"`), and deceased_npcs dialog example (`"Leo ist tot"` → `"Leo is dead"`). Python code comments containing German were left unchanged — they are never sent to models.
...
- **New NPC stub from rejected identity-reveal lacks description (`engine.py`):** When the guard rejected a rename and created a stub, the stub had no description because `npc_details` entries contain only rename/detail ops, not character introductions. Two-layer fix: (1) Extractor prompt: `npc_details` now explicitly states it is for ALREADY KNOWN NPCs only — new characters must always go into `new_npcs` with a full description. (2) Engine fallback: `world_addition` from the Brain output (which often contains a brief character introduction) is now threaded through `_apply_narrator_metadata(world_addition=...)` → `_process_npc_details(world_addition=...)` and used as description seed when the stub description would otherwise be empty. All 4 `_apply_narrator_metadata` callsites pass `brain.get("world_addition", "") or ""` (momentum-burn path uses `brain_data`).
...
### Fixed
- **`check_npc_agency()` clock ticks not logged in `session_log` (`engine.py`):** `check_npc_agency()` advances NPC-owned clocks every 5 scenes (`clock["filled"] += 1`) but previously returned only `list[str]` — prompt strings for the narrator. The fill mutations were therefore invisible in `session_log[-1]["clock_events"]`, in Elvira session JSON, and in the narrator prompt's clock context. Session analysis (v0.9.90 run, 15 turns) confirmed two silent ticks: `Jakobs Verstrickung` 1→2 at turn 4 (actual `scene_count=5`) and 2→3 at turn 15 (`scene_count=15`), both correctly triggered by the `scene_count % 5 == 0` condition but producing empty `clock_events` lists. Fix: `check_npc_agency()` return type changed from `list[str]` to `tuple[list[str], list[dict]]`. For every tick, a clock_event dict is now appended (`clock`, `trigger`, `source="npc_agency"`, `npc` fields; `fired=True` when the clock completes) and returned as the second element. All three callsites updated: main action path (`process_turn`, line ~7455) unpacks the tuple and extends `session_log[-1]["clock_events"]` after `session_log.append()`, analogous to `auto_clock_events`; both `process_correction()` callsites (input_misread re-roll and state_error action paths) likewise unpack and extend. `agency_clock_events = []` is pre-initialised in `process_correction()` so dialog-only correction paths produce no spurious extend. The tick is also now logged via `log()` at `[Clock]` level for file-logger visibility.
...
### Fixed
- **`game.director_guidance` missing `act_transition` and `npc_reflections` (`engine.py`):** `_apply_director_guidance()` applied both fields to game state (blueprint transitions, NPC memory/arc writes) but never stored them in `game.director_guidance`. The dict therefore only held `narrator_guidance`, `npc_guidance`, `pacing`, `arc_notes` — making `act_transition` and `npc_reflections` unreadable after `run_deferred_director()` returned. Session log analysis confirmed both fields were always `None` in diagnostic output. Fix: `_apply_director_guidance()` now stores `act_transition` (bool, raw from guidance) and `npc_reflections` (diagnostic subset: `npc_id`, `tone`, `tone_key`, `updated_arc` per entry) in `game.director_guidance`. The subset omits prose fields (`reflection`, `updated_description`) and write-back fields (`agenda`, `instinct`) that are already applied to NPC state and do not need to travel further. Snapshot/restore (`_build_turn_snapshot` / `_restore_from_snapshot`) and chapter-start reset (`game.director_guidance = {}`) are unaffected — `deepcopy` carries the new fields automatically.

### Changed
- **NPC `instinct` instruction reframed at both callsites (`engine.py`):** Session analysis (0.9.90 Elvira run) showed all three NPCs assigned to the same PAD quadrant (low arousal / high dominance) despite the 0.9.88 diversity fix — Haiku was finding synonyms for "calm and controlled" that were textually distinct but emotionally identical. Root cause: the previous instruction listed an emotional spectrum and named one forbidden phrase ("calm and calculating"), which Haiku treated as a synonym-block rather than a concept-block. Fix: both `call_opening_metadata()` and the Director `needs_profile` instruction now define instinct as a *psychological signature under real pressure* — what an NPC specifically does when their strategy fails, what is slightly irrational but inevitable. The spectrum list and the no-repeat constraint are removed. Concrete BAD/GOOD examples replace them, targeting the exact failure mode (role demeanor vs. human specificity). A SELF-CHECK phrase ("could this apply to any competent professional in this genre?") gives Haiku an actionable test. The opening-metadata instruction adds a genre-prior warning: situational behavior in the narration is not personality wiring — a composed character in a tense scene may have a volatile instinct underneath.
