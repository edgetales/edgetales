...
- **`WEAK_HIT` / `STRONG_HIT` / `MISS` tokens leaked into narrator prose.** The narrator system prompt listed these result types as bare uppercase tokens (`- WEAK_HIT: success at tangible cost`). Sonnet occasionally interpreted them as narrative vocabulary and used them directly in prose (e.g. *"Der WEAK_HIT zeigt sich im nächsten Moment..."*). Fix: rules reframed to reference the actual `<result type="...">` XML tag the narrator receives, making clear these are technical identifiers, not prose concepts.
- **`WOUNDED` / `BROKEN` / `DEPLETED` token leakage in `_status_context_block()`.** Same class of bug: the `health=0`, `spirit=0`, and `supply=0` branches of the status ternaries produced bare uppercase tokens as prose-visible strings (e.g. `"WOUNDED — at the limit, physical collapse imminent (flag already set)"`). Fix: replaced with lowercase natural-language descriptions that redirect to `<flags>` for the mechanical signal: `"at the physical limit — collapse is imminent (see <flags>)"`, etc.
- **`FINAL_SCENE` / `CRISIS` uppercase tokens in `<flags>` element.** These were injected as all-caps identifiers (`FINAL_SCENE:dramatic ending,...`), inconsistent with the lowercase XML convention used everywhere else and potentially leakable as prose nouns. Fix: lowercased to `final_scene:` and `crisis:`.
- **`revelation_ready` referenced as bare identifier in narrator rules.** Every other conditional rule in the narrator system prompt references its trigger as an XML tag (`<story_arc>`, `<position>`, `<pacing>`, etc.), but the revelation rule used a bare snake_case identifier (`If revelation_ready is set`). Fix: updated to `If <revelation_ready> is present` for consistency and to reduce prose-leakage risk.
