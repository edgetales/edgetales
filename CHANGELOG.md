...
### Fixed
- **Post-Completion Pacing: ewiger `phase:climax`-Loop nach Epilog-Dismiss.** Wenn ein Spieler das Epilog-Angebot dismisste und weiterspielte, blieb `get_current_act()` für immer im letzten Act (Phase `climax`) stecken, weil alle Transitions bereits getriggert waren. Folge: `_should_call_director()` feuerte **jede Szene** mit Trigger `phase:climax`, der Narrator bekam dauerhaft „Guide toward conclusion in 1-2 scenes", und der Director arbeitete endlos auf ein Finale hin, das der Spieler bewusst abgelehnt hatte. Fix: `get_current_act()` gibt bei `story_complete + epilogue_dismissed` einen synthetischen `aftermath`-Act zurück. `aftermath` ist nicht in der Phase-Trigger-Liste von `_should_call_director()`, wodurch der Director auf normalen `interval`-Rhythmus (alle 3 Szenen) zurückfällt. `_story_context_block()` emittiert in diesem Fall eine eigene `<story_ending>`-Instruktion die dem Narrator sagt, organisch zu folgen statt auf Abschluss zu drängen. `DIRECTOR_SYSTEM` erklärt `aftermath` als „Season 2 setup"-Modus (Konsequenzen, Beziehungen, neue Spannungsbögen — kein Finale erzwingen).
- **Ausgelöste Clocks kleben als Schein-Bedrohung im Director-Prompt.** Vollständig gefüllte Clocks hatten kein abgeschlossenes Flag und erschienen im Director-`<clocks>`-Block weiterhin mit vollständigem Fill-Bar als offene Bedrohung. Fix: alle vier Clock-Fill-Stellen (`apply_consequences` MISS-Pfad, `_tick_autonomous_clocks`, NPC-Scheme-Pfad) setzen jetzt `"fired": True` sobald `filled >= segments`. `build_director_prompt()` trennt aktive und gefeuerte Clocks: aktive erhalten den vollen Bar mit Prozent und Typ wie bisher; gefeuerte erscheinen nur noch als kompakte Liste unter `fired (already triggered, no longer active)`. `load_game()` setzt `fired=True` rückwirkend für bestehende Saves mit vollen Clocks ohne Flag (Backward-Compat).
- **NPC-Agenda veraltet nach story-verändernden Ereignissen.** Der Director konnte Agendas strukturell nie überschreiben — das `agenda`-Feld in `npc_reflections` füllt nur leere Felder (`needs_profile="true"`). NPCs deren Ziele sich durch Niederlage, Verrat oder Enthüllung fundamental gewandelt haben (z.B. Laparosc: „Ruf bewahren" nachdem er vollständig besiegt wurde) behielten ihre veralteten Agendas dauerhaft. Fix: zwei neue Felder `updated_agenda` und `updated_instinct` im `DIRECTOR_OUTPUT_SCHEMA` (analog zu `updated_description`, immer-überschreibend wenn non-null). `_apply_director_guidance()` wendet sie nach der fill-empty-Logik an und loggt Old→New. Prompt-Instruktion erklärt wann der Director sie nutzen soll: bei fundamentalen Zieländerungen durch Storywende — nicht bei kleinen Stimmungsverschiebungen.
- **Ortsvarianten brechen Spatial Guards und NPC-Proximity-Hints.** Der Metadata Extractor hat `location_update` manchmal mit angehängten Qualifikatoren zurückgegeben (z.B. `"Serges Heim in Lyon"` statt `"Serges Heim"`). Da alle drei Spatial-Vergleiche strikt per `==` verglichen, wurden betroffene NPCs fälschlicherweise als „abwesend" eingestuft — der Dedup-Spatial-Guard in `_description_match_existing_npc()` schloss sie aus, und die Prompt-Builder zeigten überflüssige `[at:...]`-Hints. Fix dreigleisig: (1) Neue Hilfsfunktion `_locations_match(a, b)` mit Fuzzy-Vergleich: Multi-Wort-Locations nutzen Word-Set-Subset nach Stopwort-Filterung (`"Serges Heim"` ⊆ `"Serges Heim in Lyon"` → True); Single-Wort-Locations nutzen Prefix-Match statt Subset um Städtenamen-Overflow zu verhindern (`"Lyon"` ≠ `"Serges Heim in Lyon"` weil `"lyon"` ≠ erstes Wort `"serges"`, aber `"Hintergasse"` = `"Hintergasse in Lyon"` weil `"hintergasse"` = erstes Wort). Alle drei Vergleichsstellen umgestellt. (2) `update_location()` nutzt jetzt ebenfalls `_locations_match` — Qualifier-Varianten des aktuellen Orts werden nicht als echter Ortswechsel gewertet und `current_location` bleibt kanonisch. (3) Extractor-Prompt-Instruktion für `location_update` präzisiert: bei unverändertem Ort immer null, bei Ortswechsel Ortsnamen kurz und ohne Qualifikatoren.
- **Akt-Übergänge: fehlender Guard für finalen Akt + Lücken in `triggered_transitions`.** Zwei verwandte Bugs in `_apply_director_guidance()`: (1) Wenn der Director `act_transition=true` signalisierte während er sich im finalen Akt befand (z.B. Climax mit `PAST_RANGE=true`), wurde der finale Akt trotzdem in `triggered_transitions` eingetragen — semantisch falsch, da der letzte Akt per Design keinen Transition-Trigger hat (der Loop in `get_current_act()` schließt `acts[-1]` explizit aus). Neuer Guard: bei `act_idx >= len(acts) - 1` wird das Signal ignoriert und geloggt. (2) Zwischenakte die via scene_range-Fallback übersprungen wurden (Director hat in ihrem Fenster nie `act_transition=true` gesetzt), fehlten in `triggered_transitions`, was zu Lücken führte (z.B. `['act_0', 'act_2']` statt `['act_0', 'act_1', 'act_2']`). Gameplay war nicht broken (scene_range-Fallback funktionierte weiterhin), aber die Daten waren inkonsistent. Neues Back-fill: beim Aufzeichnen eines Akts werden alle vorangehenden un-triggered Akte mit überschrittener scene_range ebenfalls markiert.
- **Deskriptor-Aliases verunreinigen die NPC-Sidebar.** Das Dedup-System akkumuliert korrekt Lookup-Aliases aus Beschreibungen die die KI vor der Namens-Enthüllung verwendet hat (z.B. `"Ein zweiter Mann in braunem Wams"`, `"Der Mann im braunen Wams"`). Diese sind für Namens-Suche und Brain-Target-Erkennung wertvoll, aber hässlich als UI-Anzeige. Bisherige Sidebar zeigte alle Aliases ungefiltert unter „auch bekannt als". Fix: neue Hilfsfunktion `_display_aliases(aliases)` in `app.py` filtert: Aliases mit >4 Wörtern, snake_case-Stubs (Unterstrich), und Artikel-gestartete Aliases mit ≥3 Wörtern (`Der Mann im braunen Wams`). Zwei-Wort-Epithets mit Artikel (`der Fass-Schläger`, `Die Schreiberin`) bleiben erhalten. Engine-intern (Brain, Director, Extractor, `_find_npc`) sehen weiterhin alle Aliases — keine Lookup-Regression.
- **`location_history`-Dedup nutzte inkonsistente Overlap-Logik.** Der Deduplizierungs-Check beim Eintragen in `location_history` verwendete einen eigenen Word-Overlap-Schwellwert (>50%), während alle anderen Ortsvergleiche nun `_locations_match()` nutzen. Die alte Logik ignorierte Stopwörter nicht und hatte einen anderen Fuzzy-Schwellwert, was zu inkonsistenten Ergebnissen führen konnte. `update_location()` nutzt jetzt `_locations_match()` für den History-Dedup, konsistent mit allen anderen Ortsvergleichen in der Engine.
- **Kishotenketsu-Finale nie als Director-Trigger erkannt.** `_should_call_director()` löste Phase-Trigger für `"climax"`, `"resolution"` und `"ten_twist"` aus, aber nicht für `"ketsu_resolution"` — die finale Phase des Kishotenketsu-Strukturtyps. Director wurde im Kishotenketsu-Finale damit nie durch die Phase getriggert, sondern lief nur über den Interval-Fallback (alle 3 Szenen). `"ketsu_resolution"` zur Trigger-Liste ergänzt; Kommentar erklärt die Symmetrie zwischen beiden Strukturtypen.
- **Deskriptor-Aliases aus `npc_merge` in Correction-Ops nicht bereinigt.** Wenn der Correction-Flow zwei NPCs zusammenführte (`npc_merge` op), wurde der `source`-Name als Alias in `target` übernommen — ohne Sanitierung. War `source` ein Deskriptor-Name (`"Ein Mann im braunen Wams"`), landete dieser unbereinigt in der Alias-Liste. `_apply_name_sanitization(target)` wird jetzt nach dem Absorb-Schritt aufgerufen, bevor `source` entfernt wird.
- **`_absorb_duplicate_npc` fehlte Alias-Sanitierung und Memory-Konsolidierung.** Der organische Duplikat-Absorb-Pfad (Metadata Extractor erkennt Identitäts-Enthüllung → `_process_npc_details()` → `_absorb_duplicate_npc()`) übernahm Aliases des absorbierten NPCs ohne `_apply_name_sanitization()` — Deskriptor-Namen konnten so als Aliases kleben bleiben, dasselbe Problem wie bei `npc_merge` im Correction-Flow. Außerdem rief der Pfad nach dem Memory-Transfer nicht `_consolidate_memory()` auf: ein NPC mit 25 Einträgen der einen weiteren NPC mit z.B. 10 Einträgen absorbiert, hatte anschließend 35 unkomprimierte Memories bis zur nächsten organischen Konsolidierung. Beide Lücken geschlossen: `_apply_name_sanitization(original)` und `_consolidate_memory(original)` werden jetzt am Ende von `_absorb_duplicate_npc()` aufgerufen.
- **`story_blueprint`-Mutationen überleben `##`-Korrektur und Momentum-Burn-Rollback nicht.** `_build_turn_snapshot()` sicherte `story_blueprint` nicht — nur NPCs, Clocks und Ressourcen wurden restauriert. Drei sub-Felder können sich während eines Turns ändern: `revealed` (wenn `mark_revelation_used()` aufgerufen wird), `triggered_transitions` (wenn Director einen Akt-Übergang signalisiert), und `story_complete` (wenn `_check_story_completion()` das Flag setzt). Bei `##`-Korrektur eines solchen Turns blieben diese Mutationen dauerhaft — eine verwendete Revelation konnte nie mehr erscheinen, ein zu früh getriggerter Akt-Übergang blieb, ein gesetztes `story_complete` löste den Epilog-Flow aus. Fix: `_build_turn_snapshot()` erfasst jetzt `bp_revealed`, `bp_triggered_transitions` und `bp_story_complete` als Listkopien; `_restore_from_snapshot()` und der Momentum-Burn-Restore-Pfad schreiben sie zurück. Backward-kompatibel: ältere Snapshots ohne diese Keys ignorieren die Restore-Schritte.
- **`_npcs_present_string` nutzte noch strict Location-Gleichheit.** Die Fallback-NPC-Stringfunktion für `build_dialog_prompt`/`build_action_prompt` (wenn `activated_npcs=None`) verwendete `npc_loc.lower() != player_loc` statt `_locations_match`. War die letzte von fünf Location-Vergleichsstellen die noch nicht auf die Fuzzy-Logik umgestellt war. Behoben.
- **Recovery-Moves gaben keine positiven Ressourcen-Konsequenzen.** `endure_harm`, `endure_stress` und `resupply` hatten in `apply_consequences()` nur MISS-Verlust-Logik, aber kein Ressourcen-Plus bei STRONG_HIT oder WEAK_HIT — obwohl das die Kernfunktion dieser Moves ist. STRONG_HIT: +1 zur entsprechenden Ressource (Health/Spirit/Supply), +2 bei `effect="great"`. WEAK_HIT: +1. Kein Consequence-Eintrag wenn Ressource bereits bei 5 (max). Die Konsequenz erscheint im `consequences`-Array und damit in der Würfel-UI und im Session-Log. Crisis-Exit-Check nach Recovery funktioniert korrekt: wenn Health durch endure_harm STRONG_HIT von 0 auf 1 steigt, wird `crisis_mode = False`.
