...
### Fixed
- **Skip-to-content link removed.** The accessibility skip link ("Zum Spielverlauf springen" / "Skip to game log") was visible on all screens because `position: absolute; top: -100%` is calculated relative to the NiceGUI wrapper div (whose height is 0), not relative to the viewport. Since EdgeTales has no extended navigation (the header contains only a single button), the skip link provides no practical value. Link rendering (`ui.html`) removed from `_show_main_phase()` and `.skip-link` CSS rule removed from `custom_head.html`. i18n keys `aria.skip_to_content` are retained.
- **Scene marker heading excessively large on desktop.** Scene markers were rendered as `<h2>` elements — the browser default stylesheet sets `h2` to ~1.5rem, causing `.scene-marker { font-size: 0.8em }` to be calculated relative to that. Fix: `<h2>` → `<div>` in both render locations (`render_chat_messages()` and `process_player_input()`). `font-size: 0.85rem` in `.scene-marker` now resolves directly against the root font size. Redundant mobile override line (`.scene-marker { font-size: 0.8em }` in the `@media` block) removed.
